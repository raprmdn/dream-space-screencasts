...
use App\Notifications\{ResetPasswordNotification, SendEmailVerification};
...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    /**
     * Send a link to verify the user's email address.
     *
     * @return void
     */
    ...
    public function scopeSearch($query, $params)
    {
        return $query->where(function ($query) use($params) {
                        $query->whereRaw('LOWER(name) like ?', '%' . strtolower($params) . '%')
                        ->orWhereRaw('LOWER(email) like ?', '%' . strtolower($params) . '%');
                    })
                    ->latest()->paginate(16)
